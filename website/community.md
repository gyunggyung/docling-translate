---
layout: page
title: Community
permalink: /community/
---

# 👥 커뮤니티 (Community)

Docling Translate는 오픈소스 프로젝트로서 커뮤니티의 참여를 환영합니다.

## 토론 및 질문 (Discussions)
궁금한 점이 있거나 아이디어를 제안하고 싶다면 GitHub Discussions를 이용해주세요.
- [GitHub Discussions 바로가기](https://github.com/gyunggyung/docling-translate/discussions)

## 이슈 제보 (Issue Tracker)
버그를 발견하거나 새로운 기능을 요청하고 싶다면 이슈 트래커에 등록해주세요.
- [GitHub Issues 바로가기](https://github.com/gyunggyung/docling-translate/issues)

## 기여하기 (Contributing)
프로젝트에 직접 기여하고 싶으신가요? 기여 가이드라인을 확인해보세요.
- [기여 가이드라인 (CONTRIBUTING.md)](https://docling-translate.readthedocs.io/ko/latest/contribution.html)

## 연락 채널 (Channels)
- **Email**: maintainer@docling-translate.org (예시)
- **Slack**: [Join our Slack Workspace](#) (준비 중)
