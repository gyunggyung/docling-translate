# Docling Translate - Project Status

이 브랜치(`gh-pages`)는 **Docling Translate** 프로젝트의 공식 웹사이트 소스 코드를 담고 있습니다.

## 🔗 주요 링크
- **공식 웹사이트**: [https://gyunggyung.github.io/docling-translate/](https://gyunggyung.github.io/docling-translate/)
- **공식 문서 (ReadTheDocs)**: [https://docling-translate.readthedocs.io/ko/latest/](https://docling-translate.readthedocs.io/ko/latest/)
- **GitHub 저장소**: [https://github.com/gyunggyung/docling-translate](https://github.com/gyunggyung/docling-translate)

---

## ✅ 완료된 작업 (Accomplished)

### 1. 문서화 (Documentation)
- **Sphinx & ReadTheDocs 통합**: `docs/` 폴더 기반의 자동화된 문서 빌드 시스템 구축.
- **콘텐츠 한글화**: `Getting Started`, `Usage`, `API Reference` 등 10개 필수 페이지 전면 한글화.
- **버전 관리**: 프로젝트 버전(v1.1.0)에 맞춘 문서 업데이트.

### 2. 웹사이트 구축 (Website)
- **GitHub Pages 배포**: `gh-pages` 고립 브랜치(Orphan Branch) 전략을 사용하여 소스 코드와 웹사이트를 분리 관리.
- **Jekyll 테마 적용**: `minima` 테마를 기반으로 커스텀 디자인 적용.
- **필수 섹션 구현**: Homepage, Feature Showcase, Community, Contact 정보 포함.

### 3. 디자인 리뉴얼 (Design Overhaul)
- **Dark Mode 테마**: `docling.ai`를 벤치마킹한 프리미엄 다크 모드(#02040A) 적용.
- **Bento Grid 레이아웃**: 기능을 직관적으로 보여주는 2열 카드 그리드 디자인 구현.
- **Hero Section 개선**: AI 생성 배경 이미지와 터미널 UI를 결합하여 개발자 친화적인 인상 제공.
- **타이포그래피 강화**: `Pretendard` 및 `JetBrains Mono` 폰트 적용으로 가독성 향상.

---

## 📝 예정된 작업 (To-Do)
- [ ] **콘텐츠 최신화**: 프로젝트 업데이트에 따른 웹사이트 정보 동기화.
- [ ] **커뮤니티 활성화**: GitHub Discussions 및 Issues 연동 강화.
- [ ] **SEO 최적화**: 검색 엔진 노출을 위한 메타 태그 및 구조화된 데이터 보강.
