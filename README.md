...
**💡 성능 팁**: `--max-workers 8` 옵션을 사용하면 번역 속도가 **4.7배 향상**됩니다! (239초 → 51초)
...
- [x] **3가지 포맷 출력**: 원문/번역문/통합본 마크다운 파일 + 인터랙티브 HTML 생성
- [x] **페이지 번호 표시**: `(p. N)` 형식으로 원본 페이지 번호 참조
- [x] **문장 단위 번역 및 대조**: 가독성 높은 비교를 위한 문장 단위 번역
- [x] **폴더 단위 번역**: 폴더 전체의 PDF를 한 번에 번역하는 기능
- [x] **성능 향상을 위한 병렬 처리**: 멀티스레딩을 활용하여 번역 속도 **4.7배 향상** 🚀
- [x] **다중 번역 엔진 지원**: Google Translate, DeepL, Gemini API 선택 가능
- [x] **다국어 번역 지원**: 한국어 외 다른 언어로의 번역 기능 확장
- [ ] **웹 UI 병렬 처리 적용**: Streamlit 앱에도 병렬 처리 통합
- [ ] **추가 번역 엔진**: GPT API, 로컬 LLM 등 최신 모델 지원
